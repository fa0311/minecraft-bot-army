...
while true; do
  ...
  echo "=== $(date -Is) foreman round, maps:$LIST audit: $(printf '%s' "$AUDIT" | head -1 | cut -c1-80) ===" >> $LOG
  PROMPT="You are the FOREMAN (現場監督) of a 50-bot Minecraft survival army in /root/workspace (runbook CLAUDE.md is loaded; docs/GOALS.md has the doctrine, the x30 SCALE RULE and the roadmap). The owner is tired of being the only one who notices problems. Do what he does: LOOK, then nag and fix.
0. $AUDIT
...
3. Judge like a demanding veteran player (start with the BASE AUDIT alerts of step 0 - each one is a measured fact with coordinates; an alert that is still there from the last round and has no job working on it is YOUR failure): floods, stray water, floating blocks/tree crowns, junk pillars, holes, unfinished walls, trees standing on fields, tiny things that should be x30, bots standing still in a clump, sites on uneven ground (one site = ONE height: level first), death causes that repeat, jobs that produce nothing, storage overflow, missing light.
