...
const JOB_JA = [[/^cavity/, '空洞埋め'], [/^fill_ravine/, '渓谷埋め'], [/^fill_|^minehead_pit/, '穴埋め'], [/^road_|^base_road|^nether_gate_road/, '道路'], [/^barter/, '物々交換'], [/^dragon_/, 'ドラゴン準備'], [/^base_depot|^depot/, '倉庫'], [/^base_infill|^base_yard|^base_field|^base_pen|^base_dorm|^base_mine/, '拠点整備'], [/^nether_|portal/, 'ネザー'], [/^village|^trade|^iron_farm/, '交易'], [/^cap_/, '土かぶせ'], [/^void_fix/, '穴埋め'], [/^armoury|^ladders/, '工作'], [/terrace|_cut/, '掘削'], [/^stray_|^canyon_/, '片付け'], [/^mine_obsidian/, '黒曜石'], [/^mine_/, '採掘'], [/^fill_ravine/, '渓谷埋め'], [/^fill_|_void/, '穴埋め'], [/^base_infill|_pad$/, '整地'], [/^base_road/, '道路'], [/^base_cane|^cane/, 'サトウキビ'], [/^base_field/, '畑づくり'], [/^base_dorm/, '寮づくり'], [/^base_/, '建築'],
...
const TYPE_JA = { build: '建築', farm: '畑', cane: 'サトウキビ', herd: '牧畜', hunt: '狩り', guard: '警備', scan: '補給係', tidy: '片付け', lumber: '伐採', fish: '釣り', scout: '偵察', steps: '手順作業', delegate: '採掘', sleeper: '就寝係', ores: '鉱石拾い', light: '照明', haul: '運搬', portal: 'ネザー', trade: '交易', muster: '待機' }
...
function labelPass () {
  try {
    ...
    for (const f of fs.readdirSync(path.join(FIELD, 'hb'))) {
      ...
      const job = String(h.job || 'muster'); const ja = (JOB_JA.find(([re]) => re.test(job)) || [])[1] || TYPE_JA[typeOf[job]] || '雑務' // never an English job id over a bot's head (owner 09-20: new job ids showed up untranslated)
