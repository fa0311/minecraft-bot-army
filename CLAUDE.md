...
0a. **An experiment goes out BARE (owner 09-21: 「実験的な試みは丸腰でさせたら？」).** A job whose design is not yet proven carries `params.bare:true`: the bot banks its diamonds, its armour and everything else of value before it starts, and works with stone tools, food and the job's materials, so a death costs nothing (115 deaths have cost 1 519 iron-equivalents and 580 diamonds; in the Nether nothing is ever recovered). The flag comes off by hand once the design has carried a crew with 0 deaths - that is what turns an experiment into a job.
...
4. **TRAVEL is read-only - and every exception is a NAMED TECHNIQUE, never a habit.** Walking somewhere never digs or places (each bot's private shortcut is the next bot's trap; the terrain guard enforces it every 2 s). What a player legitimately does to the world with his body is a technique in `lib/moves.js` or a job's own cut (owner 09-20: "移動中の地形変更はかなりの例外がありそう"): water-bucket landing + fall reflex (water is taken back), escape from a trap (pillar = the hole is filled), bridge over a void (`bridgeTo`: pathfinder scaffolding + sneak, inside a corridor, for a construction job), a job's planned shaft/stair (cavity, mine: filled or kept on purpose). Each one is time-boxed, takes the guard's opt-out WITH A REASON (`terrain_guard` logs `opt_out`), verifies the world afterwards and cleans up after itself. A new exception = a new technique there, with a live proof - not a flag flipped in a job.
