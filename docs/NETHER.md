...
Keep this under 95 lines. Machine truth: `bots/army/jobs.json → settings.nether`, `armyctl.js events 20 "portal|nether|pair"`.
...
| the_nether, the PLAN | the ideal partner of home is floor(-327/8), floor(-518/8) = **-41,-65**, but that column is open air down to the lava sea at y23 (it killed a scout on 09-20). Paper searches **16 blocks** on the Nether side, so the frame goes on the SHELF at **-41,-75** (10 blocks off, return target 82 blocks from home): `work:'pair'` walks there on rock, levels a 15x15 pad into it, builds the frame on the median ground, lights it and writes `settings.nether.paired` — then the live old frame comes down (`work:'degate'`) |
...
* **Cell census 16:2xZ (`/tmp/cells.js`, every block read back):** the far side's 28 obsidian are **two** frames, not three —
  ...
  **DEAD:** -39..-36 / y97..101 / **z-76**, 14 obsidian, **0 portal cells** — the "unnecessary gate" the owner sees. `nether_dead_frames` (`work:'degate'`) is taking those 14 home; they are more than the 10 the partner frame needs. **Measured by camera as it goes: 14 → 12 → 6 → 5 dead obsidian left (16:47Z), the live frame's 14 and its 6 burning cells untouched throughout.** A count is only believed when the bot took it in the right world: 16:46:53Z a pass reported `gate_removed {dim:'overworld', obsidianLeft:0}` — the trip had ended and the Nether coordinates were read in overworld chunk data, so a frame with five blocks standing read as finished. `gate_removed` now carries `verified`, and an unverified pass never claims the job is done. `degate` now reads the live portal cells back first and never digs an obsidian that touches one, so the way home cannot be broken; the live frame goes only after the new pair has its three verified round trips.
* **The shelf:** at the arrival level y98 the rock runs from z -88 to about z -72 (x -50..-33); everything north of that, including the ideal partner column -41,-65, is **void down to the lava sea at y23**. So nothing is built north of the shelf: the partner gate stands ON the rock at -41,-75 and the pad is levelled into it.
...
## THE ROAD OVER A VOID IS A LIBRARY CALL, AND EVERY STEP NEAR AN EDGE SNEAKS (16:53-17:0xZ, one death paid for this)
16:53:11Z Aoi **"tried to swim in lava" at the_nether -39,24,-64**: she walked off the head of my hand-written causeway at y98 and fell 74 blocks (26 iron, 9 diamond lost), after thirteen passes that had laid nothing. Three changes, in order of importance:
1. **The partner gate moved onto solid rock.** The ideal partner point -41,-65 is open air from y100 down to the lava sea at y23 — there is nothing to tunnel through there either. Paper searches **16 blocks** on the Nether side and the shelf carries rock to z-73, so the frame goes at **-41,98,-75**: 10 blocks from the ideal point (inside 16) and its return target -328,-600 is 82 blocks from the home gate (inside 128). Both directions deterministic, **no void work at all**.
2. **`moves.bridgeTo` replaces the hand-written spine** (owner 17:0xZ 「mineflyerに置きながら移動するのあるのでは？」): mineflayer-pathfinder bridges by itself given scaffolding blocks, a corridor, the terrain guard's time-boxed opt-out and sneak on every tick. A span is ≤ 6 cells, then that span is widened and railed from the spine, then the next.
3. **Sneak, proven live** (probe, Rena at -328,16,-389, a **6-block drop** one cell west): with sneak held and forward pressed into it for 3 s she moved x **-327.50 → -327.56**, `dropped 0.00`, `onGround true`, hp 20 — the server honours sneak through ViaBackwards. Every hand-driven step near an edge now uses it (`sneakStep`), and the pathfinder is never given a goal within one cell of a drop.
Brakes: a trip that bridges nothing twice goes home, three times and the job pauses itself; `maxDeaths:3` = one more death on this design and it stops by itself.
**First trip on the new site, 17:10-17:15Z:** `portal_through {pos:[-49,98,-81], offCellS:0.4, hp:20}` → walked to the site on rock (**0 `pair_span` events: nothing to bridge**) → `pair_built {at:[-41,102,-75], platform:61, platformLeft:50, overVoid:139, frameGaps:4, lit:0/6}`, hp 20, no death. **`overVoid:139` of 225 is the next lesson:** the probe took the target column's own knoll at y101, so the pad sat 4 blocks above the shelf around it. The level is now the **median ground of the whole 15x15 box** (`pair_probe` reports `medianGround`, `spread`), so the pad is levelled into the rock instead of floating over it — and the 8 obsidian of the half-built frame at y102 are left for `nether_dead_frames` to recover (it takes any obsidian with no portal cell within 1).

## WHAT KILLS OUR SCOUTS — AND WHAT DOES NOT
**The one real Nether death of this shift:** 16:44:55Z Sakura **"went up in flames"** at the_nether -36,97,-75 taking the dead frame's last column down — a lava pocket stands at -36,98,-78, two blocks from it (28 iron + 5 diamond lost). `degate` now runs `sealNear` at the WORK SITE before it touches a frame, the way an arrival has done since stage 1.

## THE OTHER DEATHS WERE NOT THE NETHER — THEY WERE `rescue`
...
| far platform (`work:'pair'`) | 7x9 | **15x15** with the gate in the middle, 3 high clear, a **2-high rail on the whole boundary** (a ghast's fireball knocks bots off, it does not kill them), 7 clear cells at each face — and **laid only where the rock carries it**: cells with no support within 3 below are dropped from the plan and counted as `overVoid`, never bridged |
| causeway (`causewayCells`) | 3 wide + rails | **5 walkable** (two lanes each way + a spare), floor 7 wide, rail on both rims — built **span by span with `moves.bridgeTo`**, then widened and railed from the spine before the next span |
