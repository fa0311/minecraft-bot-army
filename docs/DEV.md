...
3. **Escape edits only when TRAPPED** (`A.trapped`: reversible island < 40 AND the goal is off it AND 2 route failures or 20 s; a builder inside its own fill
   ...
   hole, `trap_found` names the spot once an hour); stairs are a crew thing and are back-filled behind the climber, anything left open is reported (`escape_scar`).
